...
export async function renderSuburstChart(container) {
  ...
  //! Hierarchy [type, rating, release_year, count of titles]
  const hierarchyNames = ["rating", "type", "release_year"]
  const groupedData = d3.group(data, ...hierarchyNames.map(key => d => d[key]))
  //console.log("test2: ", hierarchyNames[0])
  //console.log("get pg 13 movies in 2010: ", groupedData.get("Movie").get("PG-13"))

  const hierarchy = d3.hierarchy(transformToDataWithCounts(groupedData, hierarchyNames))
    .sum(d => d.count)
    .sort((a, b) => b.value - a.value);


  const root = d3.partition()
    .size([2 * Math.PI, Math.max(3, hierarchy.height + 1)])
    (hierarchy);
  ...
  console.log("Transformed Data:", transformToDataWithCounts(groupedData, hierarchyNames));
